...
class PID:
    ...
    def __init__(self, M, kp=1, kd=0, ki=0, umaxIn=800, eprev=0, eintegral=0,
                 max_run_ms=10000, tolerance=4, loop_maxms = 1000, loop_minms=5,
                 Name='PidX', PrintInfo = 1):
        self.kp = kp
        self.kd = kd
        self.ki = ki
        self.name = Name
    ...
    def evalu(self,value, target, deltaT):
        ...
        if self.print_info == 1:
            ...
            if self.print_cnt >= 10:
                self.print_cnt = 0
                print(self.name, 'U:', int(u/10), 'T:',int(target), 'V:', int(value))  # for debugging
        ...
        if (self.loop_pre_pos != enc_pos)and(per_us>10):
            ...
            if abs(self.realvalue-self.target) > abs(self.tolerance):
                ...
                if self.pid_type != 'init':
                    self.pid_type = 'init'
                    print(self.name, 'U:', x, 'T:',int(self.target), 'V:', int(self.realvalue))
            self.M.speed(x)
...
if __name__ == '__main__':
    ...
    m1 = Motor(m1=12, m2=13, c1=22, c2=23)
    ...
    p1 = PID(M=m1, kp=40, kd=0, ki=40, umaxIn=900,tolerance=2)    # for position PID para
