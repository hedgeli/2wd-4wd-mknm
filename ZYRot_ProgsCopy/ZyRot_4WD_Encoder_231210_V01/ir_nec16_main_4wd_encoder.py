...
from zyrot_4wd_encoder_rawPWM import rawPWM_4WD_encoder
...
encoder_4WD = rawPWM_4WD_encoder()
CMD_TIME_MS  =  200
ir_cmd_duty = 500
ir_cmd_turn_duty = 500
...
def cb(data, addr, ctrl):
    ...
    if rec_cmd == CMD_FRONT:
        ...
        max7219.disp_sym(3)
        disp_buf = 's3'
        encoder_4WD.move_lr(ir_cmd_duty, ir_cmd_duty)
        ...
        max7219.disp_sym(1)
        disp_buf = 's1'
        #mknm_noenc.move_yxw_duty(vx=500)
        encoder_4WD.move_lr(-ir_cmd_turn_duty, ir_cmd_turn_duty)
        ...
        #mknm_noenc.move_yxw_duty(vx=-500)
        encoder_4WD.move_lr(ir_cmd_turn_duty, -ir_cmd_turn_duty)
    elif rec_cmd == CMD_TURN_RIGHT:
        print('CMD_TURN_RIGHT')
        max7219.disp_sym(1)
        disp_buf = 's1'
        encoder_4WD.move_lr(-ir_cmd_turn_duty, ir_cmd_turn_duty)
    elif rec_cmd == CMD_TURN_LEFT:
        print('CMD_TURN_LEFT')
        max7219.disp_sym(2)
        disp_buf = 's2'
        encoder_4WD.move_lr(ir_cmd_turn_duty, -ir_cmd_turn_duty)
...
def main():
    ...
    max7219.disp_uppercase_char('R')
    beep_music_melody(8)
    print('Start ZYROT Mini_4WD_encoder(带编码器) 红外遥控演示.')
    print(s)
    max7219.init_max7219()
    test(1)
