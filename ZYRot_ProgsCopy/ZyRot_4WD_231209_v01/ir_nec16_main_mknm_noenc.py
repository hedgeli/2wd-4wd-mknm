...
from zyrot_mknm_noenc_rawPWM import rawPWM_MKNM
...
def cb(data, addr, ctrl):
    ...
    if rec_cmd == CMD_FRONT:
        ...
        max7219.disp_sym(3)
        disp_buf = 's3'
        mknm_noenc.move_lr(ir_cmd_duty, ir_cmd_duty)
        ...
        max7219.disp_sym(1)
        disp_buf = 's1'
        mknm_noenc.move_lr(-ir_cmd_turn_duty, ir_cmd_turn_duty)
...
def main():
    ...
    max7219.disp_uppercase_char('R')
    beep_music_melody(8)
    print('Start mini 麦克纳姆轮小车(无编码器) 红外遥控演示.')
    print(s)
    max7219.init_max7219()
    test(1)
